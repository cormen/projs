...
void BaseApp::run(int argc, const char* argv[])
{
    ...
    if (ocl::getDevice(info, cv::ocl::CVCL_DEVICE_TYPE_GPU ) == 0)
	{
		cout << "No OCL GPU device found. Try OCL CPU device...\n" ;
		if (ocl::getDevice(info, cv::ocl::CVCL_DEVICE_TYPE_CPU ) == 0)
		{
			cout << "No OCL CPU device found.\n" ;
			throw runtime_error("No OCL GPU device found."); 
		}
	}
